...
const pool = new Pool({
    connectionString: dbUrl || 'postgresql://localhost/keyauth',
...
async function initDB() {
    ...
    try {
        await client.query(`CREATE TABLE IF NOT EXISTS config (key VARCHAR(100) PRIMARY KEY, value TEXT)`);
        await client.query(`CREATE TABLE IF NOT EXISTS users (id VARCHAR(100) PRIMARY KEY, username VARCHAR(100) UNIQUE NOT NULL, password_hash TEXT NOT NULL, role VARCHAR(50) DEFAULT 'admin', created_at TIMESTAMPTZ DEFAULT NOW())`);
        await client.query(`CREATE TABLE IF NOT EXISTS license_keys (id VARCHAR(100) PRIMARY KEY, key_code VARCHAR(100) UNIQUE NOT NULL, duration_type VARCHAR(50) DEFAULT 'single', duration_days INTEGER DEFAULT 0, status VARCHAR(50) DEFAULT 'active', is_used BOOLEAN DEFAULT FALSE, note TEXT DEFAULT '', created_at TIMESTAMPTZ DEFAULT NOW(), activated_at TIMESTAMPTZ, expires_at TIMESTAMPTZ, paused_at TIMESTAMPTZ, pause_reason TEXT DEFAULT '', remaining_time_ms BIGINT, banned_at TIMESTAMPTZ, ban_reason TEXT DEFAULT '', used_at TIMESTAMPTZ, used_by TEXT DEFAULT '', ip_address TEXT DEFAULT '', hwid TEXT)`);
        await client.query(`CREATE TABLE IF NOT EXISTS logs (id VARCHAR(100) PRIMARY KEY, timestamp TIMESTAMPTZ DEFAULT NOW(), action VARCHAR(100), details TEXT)`);

        const bcrypt = require('bcryptjs');
        const ex = await client.query('SELECT id FROM users WHERE username = $1', ['admin']);
        if (ex.rows.length === 0) {
            const hash = await bcrypt.hash('admin123', 10);
            await client.query('INSERT INTO users (id, username, password_hash, role) VALUES ($1, $2, $3, $4)', [newId(), 'admin', hash, 'admin']);
        }
...
async function getConfig() {
    ...
    const cfg = {};
    r.rows.forEach(function(row) { cfg[row.key] = row.value; });
...
async function generateKeys(options) {
    options = options || {};
    var durationType = options.durationType || 'single';
    var durationDays = options.durationDays || 0;
    var count = options.count || 1;
    var note = options.note || '';
    var prefix = options.prefix || 'KEY';
    var generated = [];
    for (var i = 0; i < Math.min(count, 100); i++) {
        var keyCode = await createUniqueKeyCode(prefix);
        var id = newId();
        ...
        generated.push({ id: id, keyCode: keyCode, durationType: durationType, durationDays: durationDays, note: note, status: 'active' });
    }
...
async function getAllKeys() {
    ...
    var r = await pool.query('SELECT * FROM license_keys ORDER BY created_at DESC');
    return r.rows.map(function(key) {
        var ev = evaluateKeyStatus(key, cfg.globalPause);
        return { id: key.id, keyCode: key.key_code, durationType: key.duration_type, durationDays: key.duration_days,
            status: ev.status, currentStatus: ev.status, statusLabel: ev.label, note: key.note, createdAt: key.created_at,
...
async function deleteKey(id) {
    var r = await pool.query('DELETE FROM license_keys WHERE id=$1 RETURNING key_code', [id]);
...
async function banKey(id, reason) {
    reason = reason || '';
    var r = await pool.query("UPDATE license_keys SET status='banned', ban_reason=$1, banned_at=NOW() WHERE id=$2 RETURNING key_code", [reason, id]);
    if (!r.rows.length) return { success: false, message: 'Key tidak ditemukan.' };
...
async function unbanKey(id) {
    var r = await pool.query("UPDATE license_keys SET status='active', ban_reason='', banned_at=NULL WHERE id=$1 RETURNING key_code", [id]);
    if (!r.rows.length) return { success: false, message: 'Key tidak ditemukan.' };
...
async function pauseKey(id, reason) {
    reason = reason || '';
    var r = await pool.query('SELECT * FROM license_keys WHERE id=$1', [id]);
    if (!r.rows.length) return { success: false, message: 'Key tidak ditemukan.' };
    var key = r.rows[0];
    if (key.status === 'paused') return { success: false, message: 'Key sudah dijeda.' };
    var remainingMs = key.expires_at ? Math.max(0, new Date(key.expires_at) - new Date()) : null;
...
async function resumeKey(id) {
    ...
    var newExp = (key.remaining_time_ms && key.duration_type !== 'lifetime' && key.duration_type !== 'single')
...
async function addTimeKey(id, amount, unit) {
    unit = unit || 'days';
    var r = await pool.query('SELECT * FROM license_keys WHERE id=$1', [id]);
    if (!r.rows.length) return { success: false, message: 'Key tidak ditemukan.' };
    var key = r.rows[0];
...
async function addTimeToAllKeys(amount, unit, target) {
    unit = unit || 'days';
    target = target || 'active';
    var ms = unit === 'hours' ? amount * 3600000 : amount * 86400000;
    var secs = ms / 1000;
    var where = target === 'all' ? "duration_type NOT IN ('single','lifetime')" : "status='active' AND duration_type NOT IN ('single','lifetime')";
...
async function verifyKeyStatus(keyCode, hwid) {
    if (!keyCode) return { valid: false, message: 'Lisensi Key tidak boleh kosong!' };
    var cfg = await getConfig();
    var r = await pool.query('SELECT * FROM license_keys WHERE UPPER(key_code)=UPPER($1)', [keyCode.trim()]);
    ...
    if (key.hwid && hwid && key.hwid.trim() !== hwid.trim()) {
        return { valid: false, status: 'hwid_mismatch', message: 'Akses ditolak: Lisensi terikat ke perangkat lain.',
            key: { keyCode: key.key_code, status: 'hwid_mismatch', statusLabel: 'Perangkat Tidak Dikenal' } };
    }
    return { valid: true, status: ev.status, message: 'Lisensi Key Valid!',
...
async function redeemKey(keyCode, clientInfo) {
    ...
    var identifier = (clientInfo.identifier || 'User').trim();
    var ip = clientInfo.ip || '127.0.0.1';
    var hwid = clientInfo.hwid || null;
    if (key.hwid && hwid && key.hwid.trim() !== hwid.trim()) {
        await addLog('HWID_MISMATCH', 'Ditolak [' + key.key_code + '] HWID tidak cocok. IP: ' + ip);
        return { success: false, status: 'hwid_mismatch', message: 'Aktivasi ditolak: Lisensi terikat ke perangkat lain.' };
    }
    var now = new Date();
    if (key.duration_type === 'single') {
    ...
    var expiresAt = key.expires_at || new Date(now.getTime() + (key.duration_days || 1) * 86400000);
    ...
    await addLog('REDEEM', 'Key ' + key.key_code + ' aktif oleh ' + identifier);
    return { success: true, message: 'Lisensi BERHASIL diaktifkan!', keyInfo: { keyCode: key.key_code, durationType: key.duration_type, activatedAt: now, expiresAt: expiresAt, usedBy: identifier } };
}

async function resetKeyDevice(id) {
    var r = await pool.query("UPDATE license_keys SET hwid=NULL, ip_address='', used_by='' WHERE id=$1 RETURNING key_code", [id]);
    if (!r.rows.length) return { success: false, message: 'Key tidak ditemukan.' };
...
async function getStats() {
    var cfg = await getConfig();
    var r = await pool.query('SELECT * FROM license_keys');
    var active = 0, expired = 0, banned = 0, paused = 0, used = 0;
    r.rows.forEach(function(k) {
        var s = evaluateKeyStatus(k, cfg.globalPause).status;
        if (s === 'active') active++;
    ...
    return { total: r.rows.length, active: active, expired: expired, banned: banned, paused: paused, used: used, globalPause: cfg.globalPause };
}

async function getLogs() {
    var r = await pool.query('SELECT * FROM logs ORDER BY timestamp DESC LIMIT 200');
    return r.rows;
}

module.exports = { initDB: initDB, findUserByUsername: findUserByUsername, getConfig: getConfig, setGlobalPause: setGlobalPause, generateKeys: generateKeys, getAllKeys: getAllKeys, deleteKey: deleteKey, banKey: banKey, unbanKey: unbanKey, pauseKey: pauseKey, resumeKey: resumeKey, addTimeKey: addTimeKey, addTimeToAllKeys: addTimeToAllKeys, verifyKeyStatus: verifyKeyStatus, redeemKey: redeemKey, resetKeyDevice: resetKeyDevice, getStats: getStats, getLogs: getLogs };